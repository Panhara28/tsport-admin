import { gql, useMutation, useQuery } from '@apollo/client';
import { Button, Modal, Table } from 'react-bootstrap';
import Notiflix from 'notiflix';

const QUERY = gql`
  query roleDetail($websiteId: Int!, $roleId: Int!) {
    roleDetail(websiteId: $websiteId, roleId: $roleId) {
      id
      name
      access {
        read
        create
        edit
      }
    }
  }
`;

const MUTATION = gql`
  mutation roleManageAccess($websiteId: Int!, $roleId: Int!, $read: Boolean, $write: Boolean, $modified: Boolean) {
    roleManageAccess(websiteId: $websiteId, roleId: $roleId, read: $read, write: $write, modified: $modified)
  }
`;

type ModalProps = {
  show: boolean;
  handleClose: () => void;
  read: boolean;
  write: boolean;
  modified: boolean;
  setRead: any;
  setWrite: any;
  setModified: any;
  websiteId: number;
  roleId: number;
};

export function RenderRoleModal({
  show,
  handleClose,
  read,
  write,
  modified,
  setRead,
  setWrite,
  setModified,
  websiteId,
  roleId,
}: ModalProps) {
  const [roleManageAccess] = useMutation(MUTATION, {
    onCompleted: data => {
      if (data.roleManageAccess) {
        Notiflix.Notify.success('Access has been granted');
      }
    },
  });
  const { data, loading } = useQuery(QUERY, {
    variables: {
      websiteId,
      roleId,
    },
    fetchPolicy: 'no-cache',
    onCompleted: data => {
      setWrite(data.roleDetail.access.create);
      setRead(data.roleDetail.access.read);
      setModified(data.roleDetail.access.edit);
    },
  });

  if (loading || !data) return <div>Loading...</div>;

  const onSaveAccess = () => {
    roleManageAccess({
      variables: {
        websiteId,
        roleId,
        read,
        write,
        modified,
      },
    });
    handleClose();
  };
  return (
    <Modal show={show} onHide={handleClose} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Plugin Access {roleId}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <p>This is the decription for plugin access</p>
        <Table responsive hover striped>
          <thead>
            <tr>
              <th>Read</th>
              <th>Write</th>
              <th>Modified</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>
                <input type="checkbox" checked={read ? true : false} onChange={() => setRead(!read)} />
              </td>
              <td>
                <input type="checkbox" checked={write ? true : false} onChange={() => setWrite(!write)} />
              </td>
              <td>
                <input type="checkbox" checked={modified ? true : false} onChange={() => setModified(!modified)} />
              </td>
            </tr>
          </tbody>
        </Table>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="secondary" onClick={handleClose}>
          Close
        </Button>
        <Button variant="primary" onClick={onSaveAccess}>
          Save Changes
        </Button>
      </Modal.Footer>
    </Modal>
  );
}
