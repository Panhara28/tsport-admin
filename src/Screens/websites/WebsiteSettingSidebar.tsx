...
export function WebsiteSettingSidebar() {
  ...
  return (
    <>
      ...
      <ListGroup>
        <Link href={`/mochub/websites/${router.query.id}`}>
          <a
            className={
              style.mocSettingList + ` ${router.asPath === `/mochub/websites/${router.query.id}` ? style.active : ''}`
            }
          >
            <ListGroup.Item className="reset-list-group-item">Edit Website</ListGroup.Item>
          </a>
        </Link>
      </ListGroup>
